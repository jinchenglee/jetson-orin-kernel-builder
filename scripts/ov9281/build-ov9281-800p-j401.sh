#!/bin/bash
# Build the production OV9281 1280x800@120fps stack for the J4012:
#   1) the DT overlay (tegra234-p3767-camera-p3768-ov9281-dual-j401-800p10bit.dtbo)
#   2) the sensor module (nv_ov9281.ko) with the gain/exposure fix.
#
# Requires /home/nvidia/l4t/r36.4.7 (L4T source: kernel header, nvidia-oot
# nv_ov9281.c + camera_gpio.h, Module.symvers). The kernel is built separately
# via the repo's make_kernel.sh / make_kernel_modules.sh.
set -euo pipefail
...
# silently no-ops (patch can't resolve the header path and, with -p1 given
# explicitly, never falls back to a basename match). This one is NOT optional:
# it's the actual gain/exposure persistence fix, so let it fail the build loudly.
